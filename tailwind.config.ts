...
const config: Config = {
  content: [
    ...
    // Add any other directories or file types you might have
  ],
  theme: {
    extend: {
      backgroundImage: {
        ...
        // Ensure to include existing backgrounds if needed
      },
      colors: {
        brown: '#8B4513', // Define brown color for grid lines
      },
    },
  },
  plugins: [
      ...
      matchUtilities(
        {
          ...
          "bg-grid-small": (value: any) => ({
            backgroundImage: `url("${svgToDataUri(
              `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="8" height="8" fill="rgb(255,254,246)"><path stroke="${value.grid}" d="M0 .5H31.5V32"/></svg>`
            )}")`,
          }),
          "bg-dot": (value: any) => ({
            backgroundImage: `url("${svgToDataUri(
